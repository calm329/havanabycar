...
const { getBookingSavedEmailText, getBookingSavedEmailHtml } = require("../compose/bookingRecieved.js");
const { getReminderHTML, getReminderText } = require("../compose/bookingReminder.js");
const { getExpiryHTML, getExpiryText } = require("../compose/bookingExpiry.js");
const { getCarCancelledEmailHtml, getCarCancelledEmailText } = require("../compose/bookingCancelled.js")
const { getPaidHTML, getPaidText } = require("../compose/bookingPaid.js");
const { getRefundNextStepsHtml, getRefundNextStepsText } = require("../compose/refundProcessing.js");
const { refundHtml, refundText } = require("../compose/refundExecuted.js");
const { getRejectedHtml, getRejectedText } = require("../compose/cardRejected.js");
const { getRefundRejectedHtml, getRefundRejectedText } = require("../compose/refundRejected.js");
...
const carCancelledEmail = async (booking, reason) => {
    try {
        ...
        //Compose email
        const subject = translation.mailText[language].cancellationSubject + ' (ID: ' + booking.causale + ')';
        const mailText = getCarCancelledEmailText(translation, emailSettings, reason);
        const mailHtml = getCarCancelledEmailHtml(translation, emailSettings, reason);

        //Set mail options
        const mailOptions = {
            from: '"HAVANABYCAR" <info@havanabycar.co>',
            to: booking.driver.email,
            bcc: 'info@havanabycar.co',
            subject: subject,
            text: mailText,
            html: mailHtml
        };
...
const carConfirmedEmail = async (booking) => {
    ...
    const subject = translation.mailText[language].bookingSubjectCar + ' (ID: ' + booking.causale + ')';
    const mailText = await getBookingSavedEmailText(translation, emailSettings, booking);
    const mailHtml = await getBookingSavedEmailHtml(translation, emailSettings, booking);
...
const sendReminder = async (booking) => {
    try {
        ...
        const subject = language == "ENG" ? "REMINDER: Only 24 Hours Left" : "RECORDATORIO: Quedan solo 24 Horas";
...
const voucherAvailableEmail = async (booking) => {
    ...
    try {
        ...
        const language = booking.language;
        const subject = language == "ENG" ? "VOUCHER AVAILABE: " : "VOUCHER DISPONIBLE";
        const url = language == "ENG" ?
            "https://havanabycar.co/ENG/booking/" + booking._id :
            "https://havanabycar.co/booking/" + booking._id;
        const mailText = await getPaidText(translation, language, url, "downloadVoucher");
        const mailHtml = await getPaidHTML(translation, language, url, "downloadVoucher");
...
const refundNextStepsEmail = async (booking, refund) => {
    ...
    try {
        ...
        const language = booking.language;
        const subject = language == "ENG" ? "REFUND - Next Steps " : "Reembolso - Proximos Pasos";
        const url = language == "ENG" ?
            "https://havanabycar.co/ENG/booking/" + booking._id :
            "https://havanabycar.co/booking/" + booking._id;
        const mailText = await getRefundNextStepsText(translation, language, url, refund);
        const mailHtml = await getRefundNextStepsHtml(translation, language, url, refund);
...
const carPaidEmail = async (booking) => {
    ...
    try {
        ...
        const language = booking.language;
        const subject = language == "ENG" ? "BOOKING CONFIRMED: " : "RESERVA CONFIRMADA";
        const url = language == "ENG" ?
            "https://havanabycar.co/ENG/booking/" + booking._id :
            "https://havanabycar.co/booking/" + booking._id;
        const mailText = await getPaidText(translation, language, url, "downloadInvoice");
        const mailHtml = await getPaidHTML(translation, language, url, "downloadInvoice");
...
const informNewCC = async (cc) => {
    ...
    try {
        ...
        const mailText = "Un usuario acaba de insertar los datos de su tarjeta para la reserva " + cc.booking + "\n\nEntrar al panel de control del admin para procesar la reserva:\n\nhttps://havanabycar.co/admin";

        const mailOptions = {
            from: '"HAVANABYCAR" <info@havanabycar.co>',
            to: 'info@havanabycar.co',
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if (err) return console.log(err);
            console.log("New CC Email Sent To: info@havanabycar.co");
            return true;
        })
...
const informNewRefundData = async (causale) => {
    try {
        ...
        const mailText = "Un usuario acaba de enviar los detalles bancarios para el reembolso de la reserva " + causale + "\n\nEntrar al panel de control del admin para procesar la reserva:\n\nhttps://havanabycar.co/admin";
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if (err) return console.log(err);
            console.log("New Refund Data Email Sent To: info@havanabycar.co");
            return true;
        })
...
const informNewEvidence = async (causale) => {
    ...
    try {
        ...
        const mailText = "Un usuario acaba de cargar evidencia de pago para la reserva " + causale + "\n\nEntrar al panel de control del admin para procesar la reserva:\n\nhttps://havanabycar.co/admin";
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if (err) return console.log(err);
            console.log("New Payment Evidence Email Sent To: info@havanabycar.co");
            return true;
        })
...
const paymentRejectedEmail = async (booking) => {
    ...
    try {
        const language = booking.language;
        const subject = language == "ENG" ? "CARD PAYMENT REJECTED" : "PAGO POR TARJETA RECHAZADO";
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if (err) return console.log(err);
            console.log("Card Payment Rejected Email Sent To: ", booking.driver.email);
            return true;
        })
...
const refundRejectedEmail = async (booking) => {
    ...
    try {
        ...
        const language = booking.language;
        const subject = language == "ENG" ? "REFUND PAYMENT REJECTED" : "REEMBOLSO RECHAZADO";
        const mailText = await getRefundRejectedText(translation, language, booking._id);
        const mailHtml = await getRefundRejectedHtml(translation, language, booking._id);
