const isHigh = (ms, lowSeasons) => {
  ...
  lowSeasons.forEach((slot) => {
    if (
      ms >= new Date(slot.start).getTime() &&
      ms <= new Date(slot.finish).getTime()
    ) {
      response = "Low";
      //return response;
    }
...
const computeDiscounts = (pricing) => {
  ...
  const total =
    ...
    fuel +
    pricing.lateReturn +
    pricing.airportPickUp +
    pricing.returnInDifferentCity +
    pricing.secondDriver +
    pricing.vipAssistant;
