...
const compileBookingObj = () => {
  try {
    ...
    formFields.forEach((item) => {
      if (!item || item == "")
        throw Error(translation[getLang()].emptyFieldsError);
    });
...
const isLate = (start, finish) => {
  ...
  const dropoff = newDate.setHours(
    finish.split(":")[0],
    finish.split(":")[1],
    0
  );
...
const calculate = () => {
  ...
  let updatedPrice =
    newBooking.secondDriver == true ? startPrice + 35 : startPrice;
  if (newBooking.vipAssistant == true) updatedPrice += 80;
  if (newBooking.paymentMethod == "btcPay")
    updatedPrice = Math.round((updatedPrice / 100) * 90);
  if (newBooking.paymentMethod == "zellePay")
    updatedPrice = Math.round((updatedPrice / 100) * 95);
  if (newBooking.paymentMethod == "creditCardPay")
    updatedPrice = Math.round((updatedPrice / 100) * 110);
...
const handleVIP = () => {
  const vipCheck = document.querySelector(".option-vip i");
  newBooking.vipAssistant = !newBooking.vipAssistant;
  vipCheck.classList.toggle("active");
  calculate();
};
...
const carBookingInit = () => {
  ...
  const vip = document.querySelector(".option-vip");
  vip.addEventListener("click", () => handleVIP());
