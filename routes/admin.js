...
router.get("/bookings/:query", mw.verify, async (req, res) => {
  ...
  const bookings = await models.CubaGoldCarBooking.find(query).sort({
    date: -1,
  });
...
router.get(
  ["/ENG/booking/:id", "/booking/:id"],
  mw.isAdmin,
  async (req, res) => {
    let booking = await models.CubaGoldCarBooking.findById(req.params.id);
    ...
    const prices = {
      btcPay: carPrice.computeToBePaid(
        booking.pricing,
        booking.discounts,
        "btcPay"
      ),
      zellePay: carPrice.computeToBePaid(
        booking.pricing,
        booking.discounts,
        "zellePay"
      ),
      bankPay: carPrice.computeToBePaid(
        booking.pricing,
        booking.discounts,
        "bankPay"
      ),
      creditCardPay: carPrice.computeToBePaid(
        booking.pricing,
        booking.discounts,
        "creditCardPay"
      ),
    ...
    const fullPrice = carPrice.computeFullPrice(
      booking.pricing,
      booking.discounts,
      booking.paymentMethod
    );
    ...
    res.render(
      "booking",
      translate.text(req, "summary", { booking, finalPrice, fullPrice, prices })
    );
  }
);
...
router.get("/bookingsPaid", mw.verify, async (req, res) => {
  ...
  let bookings = await models.CubaGoldCarBooking.find(query);
  bookings = bookings.sort(
    (a, b) =>
      time.standardizeDate(a.arrangement.startString) -
      time.standardizeDate(b.arrangement.startString)
  );
  const prices = [];
  bookings.forEach((booking) => {
    const finalPrice = carPrice.computeToBePaid(
...
router.post(
  "/uploadPdf/:id",
  mw.verify,
  multer({ storage, fileFilter }).single("pdf"),
  uploadPdf
);
...
router.post("/deletePdf/:id", mw.verify, async (req, res) => {
  try {
    const target = path.join(
      ...
      "../static/pdf/",
      req.params.id,
      "/booking-CUBAGOLDCAR.pdf"
    );
    if (fs.existsSync(target)) {
      fs.unlinkSync(target);
    }

    await models.CubaGoldCarBooking.findByIdAndUpdate(req.params.id, {
