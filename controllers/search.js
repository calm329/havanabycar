...
const getOne = async (req, res) => {
    ...
    try {
        ...
        //Get Pics of the car
        const myPath = path.join(__dirname, '../static/carsPics/' + car.car_slug + '/');
        const picsInFolder = fs.readdirSync(myPath).filter(item => item != car.thumbnail_image);
        const pics = [car.thumbnail_image, ...picsInFolder];

        //Get pick up locations and Dropoff Locations
        const pickupLocationsQuery = car.vendor == "daiquiri" ? {azienda: 'daiquiri'} : {citta: req.params.city};
        const dropoffLocationsQuery = car.vendor == "daiquiri" ? {azienda: 'daiquiri'} : {citta: req.params.dropoffCity};
        ...
        //Parse dates
        const { startMS, rentalTime } = time.computeMS({startString: req.params.pickupDate, finishString: req.params.dropoffDate});
        //Calculate price
        let dailyPrice = car.price[req.params.currency][carPrice.isHigh(startMS, lowSeasons)][carPrice.timeSpan(rentalTime)] - config.geminiDiscount;
        let extraPrice = req.params.city.includes("Airport") ? 85 : 60;
        extraPrice = req.params.city == req.params.dropoffCity ? extraPrice : extraPrice + 35;
...
const getFleet = async (req, res) => {
    ...
    try {
        ...
        //Calculate price
        const prices = {
            dailyPrice: car.price[req.params.currency].Low.days45 - config.geminiDiscount
        } 
...
const available = async (req, res) => {        
    ...
    try{
        ...
        if(req.params.city != "Habana" || req.params.dropoffCity != "Habana") vendorsArray.pop();
        ...
        for(let i = 0; i < carList.length; i++){
            ...
            const dailyPrice = carList[i].price[req.params.currency][carPrice.isHigh(startMS, lowSeasons)][carPrice.timeSpan(rentalTime)] - config.geminiDiscount;
            let extraPrice = req.params.city.includes("Airport") ? 85 : 60;
            extraPrice = req.params.city == req.params.dropoffCity ? extraPrice : Number(extraPrice) + 35;
        ...
        //Apply sorting
        cars = cars.sort((a, b) => {
            if(a.priceInfo.total > b.priceInfo.total) return req.params.sort == "DESC" ? -1 : 1;
            else if (a.priceInfo.total < b.priceInfo.total) return req.params.sort == "DESC" ? 1 : -1;
            else return 0;
        })
