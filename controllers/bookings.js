...
const create = async (req, res) => {
    ...
    try {
        ...
        //Check input fields
        if (sanitize.checkString(req.body.driver.name) == false || sanitize.checkString(req.body.driver.surname) == false || sanitize.checkString(req.body.driver.country) == false || sanitize.checkPassportNumber(req.body.driver.passport) == false) {
        ...
        if (req.body.driver.birth.split(' ')[0] != String(dateParsed.getDate()) || sanitize.checkBirth(dateParsed) == false) {
            return res.status(200).json({ error: 'wrongAge' });
        }

        //Make sure that car return is AFTER car pickup
        const { startMS, finishMS, rentalTime } = time.computeMS(req.body.arrangement);
        if (startMS > finishMS) {
        ...
        //Price Object
        const fuel = 60;
        const dailyPrice = car.price[req.body.currency][carPrice.isHigh(startMS, lowSeasons)][carPrice.timeSpan(rentalTime)] - config.geminiDiscount;
        const carTotal = (dailyPrice * rentalTime);
        const lateReturn = req.body.lateReturn == true ? Number(dailyPrice) : 0;
        const airportPickUp = req.body.arrangement.city.includes("Airport") ? 25 : 0;
        const returnInDifferentCity = req.body.arrangement.city != req.body.arrangement.dropoffCity ? 35 : 0;
        ...
        const booking = {
            bookedCar, pricing, expiry, startMS,
            date, discounts, reminderTrigger,
            ...
            causale: `HC${random.generate6DigitString()}`,
        ...
        const carBooking = await models.HavanaCarBooking(booking).save();
        const url = req.body.language == "ES" ? `/resumen/${carBooking._id}/${carBooking.currency}/` : `/ENG/summary/${carBooking._id}/${carBooking.currency}/`;
...
const inform = async (req, res) => {
    try {
        ...
        const folderPay = path.join(__dirname, "../static/payments/", req.params.id);
        ...
        const paid = await models.HavanaCarBooking.findByIdAndUpdate(req.params.id, changes);
...
const reactivate = async (req, res) => {
    ...
    try {
        ...
        const updatedBooking = await models.HavanaCarBooking.findByIdAndUpdate(req.params.id, update, options);
