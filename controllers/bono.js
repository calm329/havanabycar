...
const useBono = async (req, res) => {
    try {
        ...
        const booking = await models.HavanaCarBooking.findByIdAndUpdate(req.body.bookingId, bookingChanges, options);

        const resObj = {
            finPrice: computeToBePaid(booking.pricing, booking.discounts, booking.paymentMethod),
            totPrice: computeFullPrice(booking.pricing, booking.discounts, booking.paymentMethod),
            size: bono.size
        }
